...
\title{Multimodal Comfort-Aware Human--Robot Handover: Phase-Aware Late Fusion with Few-Shot On-Site Calibration}
...
Contributions: (i) a phase-aware late-fusion comfort scorer with EMA smoothing and missing-detection decay, deployable with zero retraining of perception models; (ii) a four-stage calibration pipeline (split $\to$ cached feature extraction $\to$ DE optimize $\to$ held-out evaluate) where optimization runs in seconds per configuration on cached features; (iii) experimental evidence that pose, despite a clear standalone signal, is net-harmful under our domain shift --- an \emph{ablation that beat its parent variant} --- with a mechanism-level explanation; (iv) a teacher--student distillation demonstrating that the calibrated comfort signal supervises a $44\times$-smaller single-RGB-frame student, validated against an independent VLM-as-judge.
...
The default cannot separate abort from continue at \emph{any} $\tau \in [30, 80]$: $J = 0.0$ because sc01 outscores sc02 in absolute level. The calibrated deploy flips this ordering and opens a 42-point margin between sc01 and sc02 means; the same margin holds across LORO (sc01 = 50.2, sc02 = 92.7 across all 15 recordings). Per-recording, under the default, sc02 \emph{trends downward} during successful handovers (mean slope $-3.05$); under deploy, every test recording's slope has the correct sign (8/8). LORO slope-sign agreement on sc02 jumps from 0/8 to 7/8 and on sc03 from 1/5 to 5/5.
...
\toprule
var & objective & sc02 $\hat\beta$ & sc01 $\hat\beta$ & sc04 $\hat\beta$ & sc02 $\mu$ & sc01 $\mu$ & $J$ & gates \\
\midrule
\textbf{A} & $\bar J + \varepsilon\cdot\text{margin}$ (Eq.~\ref{eq:loss}) & $+0.10$ & $-0.54$ & $-2.82$ & 89.1 & 55.8 & 1.0 & \textbf{6/7} \\
C          & late$-$early delta of $\bar C_i$ replaces $\bar J$           & $+2.14$ & $-1.26$ & $+0.47$ & 65.8 & 49.5 & 1.0 & 5/7 \\
F          & $\bar J$ + sc02-only tanh slope                              & $+2.86$ & $+2.88$ & $+1.48$ & 73.6 & 53.5 & 1.0 & 4/7 \\
G          & $\bar J$ + mean tanh slope across scenarios                  & $-0.75$ & $-2.38$ & $-3.50$ & 81.5 & 75.5 & 1.0 & 4/7 \\
H          & G's objective on the 20-dim space                            & $-0.72$ & $-3.35$ & $-3.34$ & 80.7 & 75.2 & 1.0 & 4/7 \\
I          & sc02-weighted ($\times 3$) tanh slope + sign penalty         & $+0.89$ & $+2.61$ & $-1.48$ & 77.1 & 65.7 & 1.0 & 4/7 \\
\bottomrule
\end{tabular}}
\end{table}

Each variant fails a specific way. \textbf{C} (delta reducer in place of $\bar J$) collapses levels: DE satisfies ``sc02 delta positive'' by lifting sc02 while parking other scenarios flat near $\tau$. \textbf{F} ($\bar J$ + sc02-only tanh slope) lifts \emph{every} scenario uniformly --- classic Goodhart, with nothing in the loss telling DE to push sc01/sc04 down. \textbf{G} ($\bar J$ + all-scenario \emph{mean} tanh slope) is the cautionary tale: mean-aggregation lets DE sacrifice the sc02 minority by making sc01/sc03/sc04/sc05 fall very steeply (slopes $-2.4$ to $-3.5$) while letting sc02 drift to $-0.75$. \textbf{H} runs G's objective on the new 20-dim space --- the same failure persists, confirming the problem is objective \emph{geometry}, not parameter-space dimensionality. \textbf{I} weights sc02's tanh slope $3\times$ and adds a hard penalty on negative sc02 slope; sc02 is fixed ($+0.89$) but sc01 drifts up ($+2.61$) because DE finds it cheapest to relax the unguarded scenario. \textbf{A} --- the level-only baseline plus margin --- is the most boring objective but the strongest gate-count winner; its barely-positive sc02 slope ($+0.10$) is the cost, and slope sign is recovered post-hoc by the no-pose ablation (\S\ref{sec:nopose}). The project's negative result: a single-objective DE with 23 training recordings cannot reliably satisfy both \emph{level} and \emph{shape} gates from inside the loss without overcorrection. We promoted A and exposed the trade-off rather than hide it.
...
slope-sign agreement (test)    & 3/5  & \textbf{5/5}  & +2 \\
...
\textbf{MobileNetV3-Small} with a single-scalar regression head, \textbf{1.58\,M parameters}, ingesting one RGB frame and emitting a 0--100 comfort score --- no depth, no temporal context, no phase signal. \textbf{Headline efficiency:} $70\,\text{M}\,/\,1.58\,\text{M} \approx 44\times$ parameter compression.

\textbf{Split.} The distillation corpus merges the 31 calibration recordings with 26 additional bags collected in a second session (2026-05) for a total of \textbf{57 bags}. \code{splits/v2.json} (\code{scripts/make\_v2\_split.py}, seed 42) draws \textbf{47 train / 10 test} from this pool, with 2 test bags per scenario sampled from both sessions so the held-out test contains an explicit \emph{cross-session} component the calibration set never saw.
...
\subsection{Results: three-way agreement on the held-out 10 bags}
...
\textbf{Three findings.} (1) \textbf{The student is a faithful copy of the heuristic} (S$\leftrightarrow$H MAE 4.1 / $r$ 0.80 overall). One frame, no depth, no phase, $44\times$ fewer parameters, and the per-frame trajectory still tracks the multimodal teacher closely. (2) \textbf{Heuristic and VLM agree on trajectory direction} ($r = 0.74$) but \textbf{disagree on absolute level}: VLM systematically scores $\sim$20 points lower across all bags. This is a real calibration gap (not noise) --- shape agreement is what defends the student against having learned a scenario-leakage shortcut, but the level gap is a genuine open question about which signal should drive the controller. (3) \textbf{Cross-session generalization gap is real}: every pairwise number degrades on the 2026-05 bags, including S$\leftrightarrow$H MAE 2.9$\to$5.5 and $r$ 0.93$\to$0.68. The student inherits the distribution shift the teacher inherits from its perception backbones.
...
10 bags is suggestive, not conclusive. The student cannot see depth, history, or phase --- the lost information is the source of the residual MAE 4.1. Distillation does not validate the teacher; the VLM judge defends only against scenario-artifact leakage, not against a miscalibrated fusion teacher. The 20-point H$\leftrightarrow$V level gap is unresolved: trust the VLM and the heuristic is over-scoring; trust the heuristic and the VLM is harsh. What the recipe \emph{does} show is a practical pattern: heavy multimodal teacher + on-site few-shot calibration $\to$ dense labels $\to$ edge-deployable student that tracks the teacher (MAE 4.1 / $r$ 0.80) and agrees with an information-restricted independent judge on trajectory direction ($r$ 0.74--0.75), inverting the usual ML practice of requiring a large labeled dataset before training anything small.

%% ===================================================================
\section{Conclusion and Future Directions}
%% ===================================================================

\textbf{Contributions.} A real-time multimodal comfort-scoring pipeline that fuses pretrained facial-emotion, gaze, and pose models into a single phase-aware score, with cached-feature calibration that iterates the 20-D fusion vector in seconds. Calibrated against 31 RealSense recordings, the system achieves $J = 1.0$ at $\tau^\star = 78$ on held-out test and 90\% LORO slope-sign agreement, against 0.00\,/\,55\% under the hand-tuned default. We document a negative finding --- under our domain conditions, MediaPipe's pose domain shift makes posture net-harmful, and a no-pose ablation beat its parent on every meaningful metric. We then demonstrated a practical path from the heavy multimodal teacher ($\sim$70\,M params, GPU-tethered) to a 1.58\,M-param MobileNetV3-Small student that tracks the teacher at MAE 4.1 / $r = 0.80$, validated against an independent GPT-5.5 VLM judge ($r = 0.74$ on per-frame trajectory, after removing scenario/phase leakage that had previously made the same judge appear to disagree).
...
\textbf{Limitations.} Small corpus ($n=31$ calibration, $n=8$ held-out; 47/10 for distillation). Five scenarios, one room, one camera mount. The level/shape trade we exposed in \S\ref{sec:race} reflects our budget more than the method. The distillation inherits the teacher's calibration choices: if the teacher's $w_p = 0$ is wrong for a future site, the student faithfully reproduces it. The cross-session generalization gap (Table~\ref{tab:distill}: 2026-04 in-session $r$ 0.93 vs.\ 2026-05 cross-session $r$ 0.68) is the most actionable limitation --- the student degrades on a recording session not used for calibration. The 20-point H$\leftrightarrow$V level gap is unresolved.
